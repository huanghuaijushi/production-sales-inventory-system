...
当前系统已经具备较完整的销售、库存和批次出库链路，并且库存流水已经支持记录业务金额、成本金额和实际销售单价：
...
现有销售单创建后会锁定成品库存，发货时按批次扣减库存并生成销售出库流水。出库流水已支持 `business_amount`、`cost_amount` 和 `businessUnitPrice`，因此后续新增外部订单导入能力时，不应绕开或重写这套正式销售逻辑。
